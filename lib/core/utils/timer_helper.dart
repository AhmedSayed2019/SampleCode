...
class TimerHelper extends ChangeNotifier{
  ...
  startTimer(String? endData) {
    // log('_buildTime', '${_timer != null} --- ${ kUser?.packageEndAt == null}');

    if( endData == null /*|| _timer != null*/ )return;
    try{
      DateTime dateTime = DateConverter.isoStringToLocalDateWithOutTimer(endData);
      final difference = _daysBetween( DateTime.now(),dateTime);
      log('startTimer', '$difference ----(${DateTime.now()} -- $dateTime)');
